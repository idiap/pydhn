...
"""Test of LagrangianPipe on experimental data"""
...
def run_pipe_test(verbose=False):
    ...
    # Initialize
    pipe.set("mass_flow", mass_flow[0] / (3600))
    pipe.set("ouside_temp", t_amb[0])
    for i in range(1800):
        dp, _ = pipe._compute_delta_p(fluid, recompute_reynolds=True)
        pipe._compute_temperatures(fluid=fluid, soil=soil, t_in=t_in[0], ts_id=i)
    ...
    for i in range(len(t_in)):
        ...
        pipe.set("mass_flow", mass_flow[i] / (3600))
        pipe.set("temperature", t_in[i])
        pipe.set("inlet_temperature", t_in[i])
        pipe.set("ouside_temp", t_amb[i])
        ...
        t_0, t_1, t_avg, _, _ = pipe._compute_temperatures(
            fluid=fluid, soil=soil, t_in=t_in[i], ts_id=i
        )
    ...
    errors.append(
        rmse(
            results["PyDHN"].iloc[0:1800].values,
            results["T_Pipe_out (measured)"].iloc[0:1800].values,
        )
    )
    errors.append(
        rmse(
            results["PyDHN"].iloc[750:850].values,
            results["T_Pipe_out (measured)"].iloc[750:850].values,
        )
    )
    errors.append(
        rmse(
