...
STEPSIZE = 300. # seconds (5 minutes)
...
net.add_lagrangian_pipe(
    ...
    length=50, diameter=0.1, roughness=0.045, line="return", 
    stepsize=STEPSIZE)
net.add_lagrangian_pipe(
...
for ts_id in range(NUM_SIM_STEPS):
    ...
    # Apply schedules
    current_hyd_schedule = {name_to_edge_map[k]: v for k, v in current_hyd_schedule.items()}
    current_hx_schedule = {name_to_edge_map[k]: v for k, v in current_hx_schedule.items()}
    ...
    lagrangian_pipes_mask = net.mask("component_type", "lagrangian_pipe", condition="equality")
    manual_pipe_loss_q.append(delta_q[lagrangian_pipes_mask].sum())

    # Get a simple average temperature for all fluid in pipes
    total_fluid_temp_sum = 0.
    total_fluid_volume_sum = 0.
    for u, v in edges[lagrangian_pipes_mask]:
        component_obj = net[(u, v)]
        total_fluid_temp_sum += np.sum(component_obj._temperatures * component_obj._volumes)
