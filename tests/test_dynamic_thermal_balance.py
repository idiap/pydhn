...
class ThermalBalanceSimulationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        net.add_node(
            name="Junction1", x=1, y=1, z=0
        )  # Pipe_A Outlet / Pipe_B and Pipe_C Inlet
        net.add_node(
            name="Cons1_In", x=3, y=1.5, z=0
        )  # Pipe_B Outlet / Consumer 1 Inlet
        net.add_node(
            name="Cons2_In", x=2, y=0.5, z=0
        )  # Pipe_C Outlet / Consumer 2 Inlet
        ...
        net.add_node(
            name="Cons1_Out", x=3, y=-1, z=0
        )  # Consumer 1 Outlet / Pipe_D Inlet
        net.add_node(
            name="Cons2_Out", x=2, y=-0.5, z=0
        )  # Consumer 2 Outlet / Pipe_E Inlet
        ...
        net.add_lagrangian_pipe(
            ...
            diameter=0.1,
            roughness=0.045,
            line="return",  # IMPORTANT: line should be "return" for return pipes
            stepsize=STEPSIZE,
        )
        net.add_lagrangian_pipe(
            name="Pipe_E",
        ...
        init_producer_temp = pd.DataFrame(
            {
                "MainProd": [self.schedules["setpoint_value_hx"]["MainProd"].iloc[0]]
                * 144
            }
        )
        ...
        self.init_schedules = {
            ...
            "setpoint_value_hx": pd.concat(
                [init_producer_temp, init_consumer_demand], axis=1
            ),
        }
    ...
    def test_thermal_energy_balance(self):
        ...
        for ts_id in range(self.num_steps):
            ...
            current_hyd_schedule = (
                self.schedules["setpoint_value_hyd"].iloc[ts_id].to_dict()
            )
            current_hx_schedule = (
                self.schedules["setpoint_value_hx"].iloc[ts_id].to_dict()
            )

            current_hyd_schedule = {
                name_dict[k]: v for k, v in current_hyd_schedule.items()
            }
            current_hx_schedule = {
                name_dict[k]: v for k, v in current_hx_schedule.items()
            }

            self.net.set_edge_attributes(
                current_hyd_schedule, name="setpoint_value_hyd"
            )
            self.net.set_edge_attributes(current_hx_schedule, name="setpoint_value_hx")

            # We need to pass ts_id for this to work
            _ = base_loop.execute(
                net=self.net, fluid=self.fluid, soil=self.soil, ts_id=ts_id
            )
            ...
            consumer1_delta_q = self.net[("Cons1_In", "Cons1_Out")][
                "delta_q"
            ]  # Energy extracted by Cons1
            consumer2_delta_q = self.net[("Cons2_In", "Cons2_Out")][
                "delta_q"
            ]  # Energy extracted by Cons2

            energy_to_consumers = (
                consumer1_delta_q + consumer2_delta_q
            )  # These should be negative values

            # Pipe heat losses
            # Iterate through pipes and sum up their heat losses for this step
            current_pipe_losses = 0.0
            ...
            balance_sum = (
                ...
                + (  # This is negative
                    -delta_Q_storage
                )  # Change in internal energy, opposite sign for balance
            )

            print(f"Producer Energy Supplied: {energy_from_producer:.3f} J")
            print(
                f"Consumer Energy Extracted: {energy_to_consumers:.3f} J (Sum of negative values)"
            )
        ...
        print(
            f"Total Producer Energy Supplied: {np.sum(producer_energy_supplied):.3f} J"
        )
        print(
            f"Total Consumer Energy Extracted: {np.sum(consumer_energy_extracted):.3f} J"
        )
        ...
        print(
            f"Average Balance Error per Step: {np.mean(np.abs(total_balance_errors)):.3e} J"
        )
        print(
            f"Max Balance Error per Step: {np.max(np.abs(total_balance_errors)):.3e} J"
        )
